...
describe("EmoteEvent", () => {
    ...
    it("should locate a stable markup block", () => {
        const block = new EmoteEvent({...templateEvent, content: {"m.markup": [{body: "test"}]}});
    ...
    it("should locate an unstable markup block", () => {
        const event = new EmoteEvent({...templateEvent, content: {"org.matrix.msc1767.markup": [{body: "test"}]}});
    ...
    it("should error if a markup block is not present", () => {
        expect(() => new EmoteEvent({...templateEvent, content: {no_block: true} as any})).toThrow(
            new InvalidEventError("m.emote", "schema does not apply to m.markup or org.matrix.msc1767.markup"),
        );
    });
});

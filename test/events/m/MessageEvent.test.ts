...
import {InvalidEventError, MessageEvent} from "../../../src";
...
describe("MessageEvent", () => {
    ...
    it("should locate a stable markup block", () => {
        const block = new MessageEvent({...templateEvent, content: {"m.markup": [{body: "test"}]}});
    ...
    it("should locate an unstable markup block", () => {
        const event = new MessageEvent({...templateEvent, content: {"org.matrix.msc1767.markup": [{body: "test"}]}});
    ...
    it("should error if a markup block is not present", () => {
        expect(() => new MessageEvent({...templateEvent, content: {no_block: true} as any})).toThrow(
            new InvalidEventError("m.message", "schema does not apply to m.markup or org.matrix.msc1767.markup"),
        );
    });
});
